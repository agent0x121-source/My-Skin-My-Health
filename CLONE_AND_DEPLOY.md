# The Dental Solutions — Clone & Deploy (the reliable one-shot)
...
1. **Brand name:** find `The Dental Solutions` (and standalone `Lumora`) across all `.html` → your business name.
...
4. **Phone / WhatsApp:** find `9307512816` (and `919765407679` if present) → your number (the lead
   form opens a prefilled WhatsApp on submit — handled by the inline `lumoraLead` script before
   `</body>` on each index page).
5. **Email:** find `hello@drkaustubhpatilpune.com` → your email.
